...
|---------|-------|-------------|------------|
| **spawn** | `[amount] [world [x y z]] [--name <name>] [--random-name] [--notp]` | Spawn one or more fake player bots | `fpp.spawn` (admin) / `fpp.spawn.user` (user) |
...
| **move** | `<bot\|all> --to <player>  \|  <bot\|all> --coords <x> <y> <z> [alias: --pos]  \|  <bot\|all> --roam [x,y,z] [radius]  \|  <bot\|all> --stop` | Navigate a bot to a player or roam randomly | `fpp.move` |
...
| **attack** | `<bot> [--mob [type]] [--range <n>] [--stop]  \|  <bot> --hunt [--range <n>] [--stop]` | Attack nearby entities or hunt mobs | `fpp.attack` |
| **follow** | `<bot\|all> <player\|--start>  \|  <bot\|all> --stop` | Make a bot continuously follow a player | `fpp.follow` |
| **find** | `<bot> <block> [--radius <n>] [--count <n>] [--prefer-visible]  \|  <bot> --stop  \|  --stop` | Path to and mine nearby blocks of a chosen type | `fpp.find` |
